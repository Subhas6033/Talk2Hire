...
const User = {
  ...
  async updateProfile(userId, { fullName, mobile, location, skills }) {
    ...
    if (updates.length === 0) throw new APIERR(400, "No fields to update");
  ...
  async updateSkills(userId, skillsString) {
    if (!userId) throw new APIERR(400, "User ID is required");
    await pool.execute(
      "UPDATE users SET skills = ?, updated_at = NOW() WHERE id = ?",
      [skillsString, userId],
    );
  },
  ...
  async updateCvExtractedSkills(userId, skillsArray) {
    ...
    await pool.execute(
      "UPDATE users SET cv_extracted_skills = ?, updated_at = NOW() WHERE id = ?",
  ...
  async updateInterviewSkills(userId, skillsArray) {
    ...
    if (!Array.isArray(skillsArray))
      throw new APIERR(400, "Skills must be an array");
    await pool.execute(
      "UPDATE users SET interview_selected_skills = ?, updated_at = NOW() WHERE id = ?",
      [JSON.stringify(skillsArray), userId],
    );
  },
