      ...
      <CardHeader headerClass="text-center">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-xs text-white/60">
            Skills from your CV • Suggested skills • Custom skills
          </p>
        </div>
      </CardHeader>
      ...
      <CardBody>
        ...
        {selectedSkills.length > 0 && (
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2 p-4 bg-linear-to-br from-purple-500/5 via-pink-500/5 to-purple-500/5 rounded-xl border border-purple-400/20 max-h-40 overflow-y-auto shadow-sm">
            ...
            <div className="mb-4 space-y-3">
              ...
              {cvSkillsInSearch.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 p-3 bg-linear-to-br from-slate-500/5 via-slate-600/5 to-slate-500/5 rounded-xl border border-slate-400/20 shadow-sm max-h-64 overflow-y-auto">
        ...
        {!inputValue.trim() && (
          <>
            ...
            {suggestedSkillsToShow.length > 0 && (
              <div className="mb-4">
                <p className="text-xs font-semibold text-white/90 mb-3 flex items-center gap-2">
                  ...
                  Suggested for You ({suggestedSkillsToShow.length}
            ...
            {popularSkillsToShow.length > 0 && (
              <div>
                <p className="text-xs font-semibold text-white/90 mb-3 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 bg-linear-to-r from-slate-400 to-slate-500 rounded-full"></div>
                  Popular Skills ({popularSkillsToShow.length})
                </p>
                <div className="flex flex-wrap gap-2 p-4 bg-white/5 rounded-xl border border-white/10 shadow-sm">
                  {popularSkillsToShow.map((skill) => (
                    <button
                      key={skill}
                      type="button"
                      onClick={() => addSkill(skill)}
            ...
            {cvSkills.length === 0 && (
              <div className="mb-4 p-4 bg-linear-to-r from-amber-500/10 to-orange-500/10 border border-amber-400/30 rounded-xl shadow-sm">
                <p className="text-xs text-amber-300 text-center font-medium">
                  💡 No skills extracted from your CV yet. Select from popular
                  skills or add custom ones.
                </p>
              </div>
            )}
          </>
        ...
        <p className="mt-3 text-xs text-white/40 text-center">
          {cvSkills.length > 0
            ? "Select from your CV skills, suggested skills, or add custom ones"
